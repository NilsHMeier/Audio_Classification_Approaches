"""
Taken from https://github.com/Akashdesarda/ResNet-builder
"""
from typing import *
import tensorflow as tf
from tensorflow.keras.applications.resnet_v2 import *
from tensorflow.keras.layers import Input, Conv2D, Dense, Activation, BatchNormalization, AveragePooling2D, Flatten, \
    Dropout
...
def residual_block(X, num_filters: int, stride: int = 1, kernel_size: int = 3,
    ...
    """
    conv_layer = Conv2D(num_filters, kernel_size=kernel_size, strides=stride, padding='same',
...
def build_resnet_model(input_shape: Tuple[int, int, int], depth: int, num_classes: int) -> Model:
    """
    ...
    for stage in range(3):
        for unit_res_block in range(num_res_block):
            ...
            y = residual_block(X, num_filters=num_filters_in, kernel_size=1, stride=stride, activation=activation,
                               bn=bn, conv_first=False)
            y = residual_block(y, num_filters=num_filters_in, conv_first=False)
            y = residual_block(y, num_filters=num_filters_out, kernel_size=1, conv_first=False)
            if unit_res_block == 0:
                # linear projection residual shortcut connection to match changed dims
                X = residual_block(X=X, num_filters=num_filters_out, kernel_size=1, stride=stride, activation=None,
                                   bn=False)
...
def build_resnet_pretrained(base_model: str = 'ResNet50V2',
    ...
    Use this if you want to use Transfer learning approach. For Fine-Tuning Layers can be freezed & only Fully connected
    ...
    freeze : bool, optional
        Freeze all convolution layers and train only on Fully connected layer. Keep it true for transfer learning, by
        default True
